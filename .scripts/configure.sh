...
if ! command -v npm &> /dev/null; then
  ...
  if [ "$NEEDS_UPDATE" = true ]; then
    echo "Attempting to upgrade clasp (found unstable or outdated version)..."
    # The ENOTEMPTY error often occurs with `npm uninstall`. A more robust method
    # is to find the global node_modules root and manually delete the package folder.
    NPM_ROOT=$(npm root -g)
    CLASP_PATH="$NPM_ROOT/@google/clasp"
    if [ -d "$CLASP_PATH" ]; then
      echo "Forcefully removing existing clasp installation at '$CLASP_PATH'..."
      sudo rm -rf "$CLASP_PATH"
    fi

    echo "Installing the latest STABLE version of '@google/clasp'..."
    # Use '@latest' explicitly to get the latest stable release.
    if ! sudo npm install -g @google/clasp@latest; then
      echo "ERROR: Failed to install '@google/clasp' globally via npm." >&2
...
if command -v clasp &> /dev/null && clasp login --status &>/dev/null; then
  ...
  if [ -z "$APP_SCRIPT_ID" ]; then
    # If not in .env, check if it exists in .clasp.json from a previous setup.
    if [ -f ".clasp.json" ] && SCRIPT_ID_FROM_FILE=$(jq -r .scriptId .clasp.json) && [ -n "$SCRIPT_ID_FROM_FILE" ] && [ "$SCRIPT_ID_FROM_FILE" != "null" ]; then
      echo "Found existing scriptId in .clasp.json. Updating .env file..."
      APP_SCRIPT_ID=$SCRIPT_ID_FROM_FILE
      export APP_SCRIPT_ID # Make it available to the current session
      echo -e "\n# Auto-generated by configure.sh\nAPP_SCRIPT_ID=\"$APP_SCRIPT_ID\"" >> .env
    else
      # Only if the ID is in neither .env nor .clasp.json, prompt to create one.
      echo "-------------------------------------------------------------------" >&2
      echo "INFO: The 'APP_SCRIPT_ID' is not set in your .env file." >&2
      read -p "Would you like to create a new Google Apps Script project now? (y/N) " -n 1 -r
      echo # Move to a new line
      if [[ $REPLY =~ ^[Yy]$ ]]; then
        echo "Creating a new Apps Script project for Google Slides..."
        # Create the project, which generates .clasp.json with the new scriptId
        if ! clasp create --type slides --title "Slide Generator (clasp)" --rootDir "apps-script"; then
          echo "ERROR: Failed to create new Apps Script project via clasp." >&2
          return 1
        fi
        # Extract the new scriptId from the generated .clasp.json file
        APP_SCRIPT_ID=$(jq -r .scriptId .clasp.json)
        if [ -z "$APP_SCRIPT_ID" ] || [ "$APP_SCRIPT_ID" == "null" ]; then
          echo "ERROR: Could not extract scriptId from .clasp.json after creation." >&2
          return 1
        fi
        echo "New project created with ID: $APP_SCRIPT_ID"
        echo "Saving this ID to your .env file for future use..."
        echo -e "\n# Auto-generated by configure.sh\nAPP_SCRIPT_ID=\"$APP_SCRIPT_ID\"" >> .env
      else
        echo "Skipping Apps Script setup. You can manually set APP_SCRIPT_ID in .env later." >&2
        return 1 # Stop sourcing the script as clasp commands will fail
      fi
      echo "-------------------------------------------------------------------" >&2
    fi
  fi
